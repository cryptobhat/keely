...
object SwipeAlgorithms {
    ...
    /**
     * Prune dictionary candidates by start/end keys and path length
     * Based on FlorisBoard's two-stage pruning approach
     */
    data class PruningCriteria(
        val startKeyChar: Char?,
        val endKeyChar: Char?,
        val pathLength: Float,
        val lengthTolerance: Float = LENGTH_THRESHOLD
    )

    /**
     * Check if a word matches pruning criteria
     */
    fun matchesPruningCriteria(
        word: String,
        idealPathLength: Float,
        criteria: PruningCriteria
    ): Boolean {
        // Check start key if specified
        if (criteria.startKeyChar != null &&
            word.firstOrNull()?.lowercaseChar() != criteria.startKeyChar.lowercaseChar()) {
            return false
        }

        // Check end key if specified
        if (criteria.endKeyChar != null &&
            word.lastOrNull()?.lowercaseChar() != criteria.endKeyChar.lowercaseChar()) {
            return false
        }

        // Check path length tolerance
        val lengthDiff = abs(idealPathLength - criteria.pathLength)
        if (lengthDiff > criteria.lengthTolerance) {
            return false
        }
